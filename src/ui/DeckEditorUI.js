...
var DeckEditorUI = {
  ...
  moveSelectedCard: function () {
    ...
    if (cardLocation === 'deck') {
      ...
      selectedCard.one('transitionend', function () {
        // 动画结束后移动卡片
        self.moveCard(self.selectedCard.uniqueId, 'deck', 'inventory');
      ...
      selectedCard.one('transitionend', function () {
        ...
        if (self.wouldExceedMaxSameCard(self.selectedCard.name, 'deck')) {
          alert('同名卡片【' + self.selectedCard.name + '】不能超过5张！');
          // 重新渲染界面并重新绑定事件
          self.render();
          self.bindEvents();
          return;
        }
        self.moveCard(self.selectedCard.uniqueId, 'inventory', 'deck');
  ...
  disassembleSelectedCard: function () {
    ...
    if (cardLocation === 'deck' || cardLocation === 'inventory') {
      ...
      selectedCard.one('transitionend', function () {
        // 从对应的列表中移除卡片
        self.removeCard(self.selectedCard.uniqueId, cardLocation);

        // 增加星点值
        if (!self.gameData.stars) {
          self.gameData.stars = 0;
        }
        self.gameData.stars += starValue;

        // 清除选中状态
        self.selectedCard = null;
        self.selectedCardUniqueId = null;
